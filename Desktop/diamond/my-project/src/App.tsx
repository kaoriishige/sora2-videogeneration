  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-40 selection:bg-yellow-600/30">
      ...
      <main className="max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="flex justify-center gap-6 mb-16">
            {['tech', 'merit'].map((tab) => (
              <button 
                ...
                className={`px-10 py-5 text-sm font-black tracking-widest transition-all duration-300 rounded-full border-2 ${activeTab === tab ? 'bg-yellow-600 border-yellow-600 text-black shadow-[0_0_20px_rgba(202,138,4,0.4)]' : 'bg-transparent border-white/10 text-gray-500 hover:text-white'}`}
              >
                {tab === 'tech' ? '独自技術の証明' : '圧倒的な導入メリット'}
              </button>
            ))}
          </div>
        ...
        <section className="mb-40">
          ...
          <div className="grid grid-cols-1 gap-16 mb-24">
            ...
            <div className="bg-[#0c0c0c] rounded-[4rem] p-10 md:p-20 border border-white/10 shadow-2xl flex flex-col lg:flex-row items-center gap-20 transition-all hover:border-yellow-600/40 group">
              ...
              <div className="w-full lg:w-1/2 space-y-10">
                <div className="space-y-2">
                  <h3 className="text-5xl md:text-6xl font-black italic tracking-tighter">ダイヤモンド〇〇</h3>
                ...
                <div className="bg-black p-10 rounded-[2.5rem] border border-white/5 shadow-inner">
                  ...
                  <input 
                    ...
                    className="bg-transparent border-b-4 border-yellow-600 text-7xl font-mono text-yellow-500 w-full outline-none text-center focus:border-white transition"
                  />
                </div>
            ...
            <div className="bg-[#0c0c0c] rounded-[4rem] p-10 md:p-20 border border-white/10 shadow-2xl flex flex-col lg:flex-row items-center gap-20 transition-all hover:border-blue-600/40 group">
              ...
              <div className="w-full lg:w-1/2 space-y-10">
                <div className="space-y-2">
                  <h3 className="text-5xl md:text-6xl font-black italic tracking-tighter text-gray-300">ブレーンウォーター</h3>
                ...
                <div className="bg-black p-10 rounded-[2.5rem] border border-white/5 shadow-inner">
                  ...
                  <input 
                    ...
                    className="bg-transparent border-b-4 border-white/20 text-7xl font-mono text-white w-full outline-none text-center focus:border-blue-600 transition"
                  />
                </div>
                <div className="flex justify-between items-center text-sm font-black px-4">
          ...
          {/* 最終決済サマリー */}
          <div className="max-w-3xl mx-auto bg-white text-black p-16 md:p-24 rounded-[5rem] shadow-[0_50px_150px_rgba(0,0,0,0.9)] relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-4 bg-yellow-600"></div>
            <div className="space-y-10 font-black">
              <div className="flex justify-between border-b-2 border-gray-100 pb-6 text-sm tracking-widest uppercase text-gray-400">
                <span>商品合計（税抜/税込込）</span>
                <span>¥{(diamondSubtotal + brainWaterSubtotal).toLocaleString()}</span>
              </div>
              <div className="flex justify-between border-b-2 border-gray-100 pb-6 text-sm tracking-widest uppercase text-red-600">
                <span>送料合計</span>
                <span>¥{(diamondShipping + brainWaterShipping).toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-end pt-12">
                <span className="text-3xl italic tracking-tighter">お支払総額</span>
                <div className="text-right">
                  <span className="text-7xl md:text-8xl font-mono tracking-tighter underline decoration-[12px] decoration-yellow-600/30">
        ...
        {/* 購入申し込みフォーム */}
        <section className="bg-[#0a0a0a] p-12 md:p-24 rounded-[5rem] border border-white/5 relative overflow-hidden">
          <div className="absolute -top-24 -right-24 w-64 h-64 bg-yellow-600/5 blur-[100px]"></div>
          
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-24">
              <h2 className="text-5xl font-black italic mb-8 tracking-tighter text-white">購入申し込み</h2>
              <p className="text-gray-500 text-lg leading-relaxed font-bold max-w-2xl mx-auto">
                下記フォームに、ご購入情報とお客様情報をご入力ください。
              </p>
            </div>

            <form id="order-form" onSubmit={handleSubmit} className="space-y-12 font-black">
              {/* 購入商品情報 */}
              <div className="bg-[#111] p-10 md:p-16 rounded-[3rem] border border-white/10">
                <h3 className="text-2xl font-black mb-10 text-yellow-600">ご購入商品</h3>
                
                <div className="space-y-8">
                  <div className="flex justify-between items-center pb-8 border-b border-white/10">
                    <div>
                      <p className="text-white text-lg mb-2">ダイヤモンド〇〇</p>
                      <p className="text-gray-500 text-sm">本体価格: ¥11,000 × {diamondQty}本</p>
                    </div>
                    <p className="text-2xl font-mono text-yellow-600">¥{diamondSubtotal.toLocaleString()}</p>
                  </div>

                  <div className="flex justify-between items-center pb-8 border-b border-white/10">
                    <div>
                      <p className="text-white text-lg mb-2">ブレーンウォーター</p>
                      <p className="text-gray-500 text-sm">本体価格: ¥2,000 × {brainWaterQty}本</p>
                    </div>
                    <p className="text-2xl font-mono text-white">¥{brainWaterSubtotal.toLocaleString()}</p>
                  </div>

                  <div className="flex justify-between items-center pb-8 border-b border-white/10">
                    <p className="text-white">送料計</p>
                    <p className="text-2xl font-mono text-red-600">¥{(diamondShipping + brainWaterShipping).toLocaleString()}</p>
                  </div>

                  <div className="flex justify-between items-center">
                    <p className="text-2xl font-black text-white">お支払総額</p>
                    <p className="text-4xl font-mono text-yellow-600">¥{totalAmount.toLocaleString()}</p>
                  </div>
                </div>
              </div>
              ...
              {/* お客様情報 */}
              <div className="bg-[#111] p-10 md:p-16 rounded-[3rem] border border-white/10">
                <h3 className="text-2xl font-black mb-10 text-yellow-600">お客様情報</h3>

                <div className="grid md:grid-cols-2 gap-10 mb-10">
                  <div className="space-y-4">
                    <label className="text-xs text-yellow-600 uppercase tracking-widest ml-4">貴社名 / 屋号</label>
                    <input type="text" name="company" placeholder="株式会社 〇〇" value={formData.company} onChange={handleFormChange} className="w-full bg-white/5 border border-white/10 p-6 rounded-2xl outline-none focus:border-yellow-600 focus:bg-white/10 transition-all text-white" />
                  </div>
                  <div className="space-y-4">
                    <label className="text-xs text-yellow-600 uppercase tracking-widest ml-4">ご担当者名（必須）</label>
                    <input type="text" name="name" placeholder="山田 太郎" value={formData.name} onChange={handleFormChange} className="w-full bg-white/5 border border-white/10 p-6 rounded-2xl outline-none focus:border-yellow-600 focus:bg-white/10 transition-all text-white" required />
                  </div>
                </div>
                
                <div className="space-y-4 mb-10">
                  <label className="text-xs text-yellow-600 uppercase tracking-widest ml-4">メールアドレス（必須）</label>
                  <input type="email" name="email" placeholder="contact@example.co.jp" value={formData.email} onChange={handleFormChange} className="w-full bg-white/5 border border-white/10 p-6 rounded-2xl outline-none focus:border-yellow-600 focus:bg-white/10 transition-all text-white" required />
                </div>

                <div className="space-y-4 mb-10">
                  <label className="text-xs text-yellow-600 uppercase tracking-widest ml-4">電話番号（必須）</label>
                  <input type="tel" name="phone" placeholder="03-XXXX-XXXX" value={formData.phone} onChange={handleFormChange} className="w-full bg-white/5 border border-white/10 p-6 rounded-2xl outline-none focus:border-yellow-600 focus:bg-white/10 transition-all text-white" required />
                </div>

                <div className="space-y-4">
                  <label className="text-xs text-yellow-600 uppercase tracking-widest ml-4">ご質問・ご要望（任意）</label>
                  <textarea rows={5} name="memo" placeholder="ご不明な点やご要望がございましたら、こちらにご記入ください。" value={formData.memo} onChange={handleFormChange} className="w-full bg-white/5 border border-white/10 p-6 rounded-2xl outline-none focus:border-yellow-600 focus:bg-white/10 transition-all text-white resize-none"></textarea>
                </div>
              </div>
